...
if uploaded_file is not None:
    ...
    for col in df.columns:
        c_low = col.lower()
        # Explicit priority for (0-1) percentage columns visible in LongoMatch/Xml exports
        ...
        # Secondary backup check if explicit tags aren't present
        elif 'x1' not in rename_dict.values() and c_low in ['x1', 'x start', 'x_start', 'start x', 'start x (m)', 'pos x']: rename_dict[col] = 'x1'
        elif 'y1' not in rename_dict.values() and c_low in ['y1', 'y start', 'y_start', 'start y', 'start y (m)', 'pos y']: rename_dict[col] = 'y1'
        elif 'x2' not in rename_dict.values() and c_low in ['x2', 'x end', 'x_end', 'end x', 'end x (m)', 'pos x2']: rename_dict[col] = 'x2'
        elif 'y2' not in rename_dict.values() and c_low in ['y2', 'y end', 'y_end', 'end y', 'end y (m)', 'pos y2']: rename_dict[col] = 'y2'
